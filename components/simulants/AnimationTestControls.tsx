...
import {
  ...
  useIsClient,
} from "../../utils/useSSRSafeAnimations";
...
export default function AnimationTestControls({
  ...
  // Component state - all useState hooks first
  ...
  // Create stable simulant array to avoid dependency issues
  const simulantArray = useMemo(() => {
    return Array.from(simulants.values());
  }, [simulants.size]); // Use size instead of the Map itself

  // Create stable simulant list for dropdown
  const simulantList = useMemo(() => {
    return simulantArray.map((simulant) => ({
      id: simulant.id,
      name: simulant.name,
  ...
  // Get current simulant using stable dependency
  const currentSimulant = useMemo(() => {
    if (!selectedSimulant) return null;
    return simulantArray.find(s => s.id === selectedSimulant) || null;
  }, [selectedSimulant, simulantArray]);
  ...
  // Simple performance tracking without the problematic hook
  ...
  }, [currentSimulant, selectedSimulant, enableRealTimeUpdates]);
  ...
  const handlePlayAnimation = useCallback(
    (animation: AnimationState) => {
      ...
      try {
        ...
        // Call callback
        onAnimationChange?.(selectedSimulant, animation);

        console.log(
          `🎬 Animation changed: ${animation} for simulant ${selectedSimulant}`,
        );
      } catch (error) {
        const errorMessage =
                ...
                {AVAILABLE_ANIMATIONS.map((animation) => {
                  ...
                  return (
                    <Button
                      ...
                      variant="ghost"
                      size="sm"
                      onClick={() => handlePlayAnimation(animation.key)}
